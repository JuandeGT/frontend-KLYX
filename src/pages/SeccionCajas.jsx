...
const SeccionCajas = () => {
	const adminCajas = useAdminCajas();
	const adminObjetos = useAdminObjetos();
	...
	const cargar = async () => {
		...
		const [dataCajas, dataObjetos] = await Promise.all([
			adminCajas.getCajas(),
			adminObjetos.getObjetos(),
		]);
	...
	const guardar = async () => {
		...
		const respuesta = editandoId
			? await adminCajas.actualizarCaja(editandoId, datos)
			: await adminCajas.crearCaja(datos);
		if (respuesta !== null) { cancelar(); cargar(); }
	};

	const eliminar = async () => {
		const respuesta = await adminCajas.eliminarCaja(confirmarEliminarId);
	...
	const añadirObjeto = async () => {
		...
		const respuesta = await adminCajas.añadirObjetoACaja(gestionandoCajaId, Number(nuevoObjetoId), Number(nuevaProbabilidad));
		if (respuesta !== null) { setNuevoObjetoId(''); setNuevaProbabilidad(''); cargar(); }
	};

	const quitarObjeto = async (objetoId) => {
		const respuesta = await adminCajas.quitarObjetoDeCaja(gestionandoCajaId, objetoId);
		if (respuesta !== null) cargar();
	};

	const cajaGestionada = cajas.find((c) => c.id === gestionandoCajaId);

	if (adminCajas.cargando && cajas.length === 0) return <Cargando />;
						...
						<div className="admin-modal-cuerpo">
							...
							<div className="admin-añadir-objeto">
								...
								<div className="admin-añadir-fila">
									...
									/>
									<button className="btn-admin-guardar" onClick={añadirObjeto} disabled={adminCajas.cargando}>Añadir</button>
								</div>
							</div>
						</div>
					</div>
				...
				<div className="admin-form-inline">
					...
					<div className="admin-form-grid">
						...
						<div className="campo-form">
							...
							<select value={formulario.vip ? 'si' : 'no'} onChange={(e) => setFormulario({ ...formulario, vip: e.target.value === 'si' })}>
					...
					<div className="admin-form-acciones">
						...
						<button className="btn-admin-guardar" onClick={guardar} disabled={adminCajas.cargando}>
							{adminCajas.cargando ? 'Guardando...' : (editandoId ? 'Guardar cambios' : 'Crear caja')}
						</button>
					</div>
				</div>
			)}
						...
						{cajas.map((c) => (
							<tr key={c.id}>
								...
								<td className="td-acciones">
									...
									<button className="btn-accion btn-eliminar" onClick={() => setConfirmarEliminarId(c.id)}>Eliminar</button>
								</td>
							</tr>
						))}
					</tbody>
