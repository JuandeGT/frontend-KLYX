...
const SeccionUsuarios = () => {
	const { getUsuarios, actualizarUsuario, eliminarUsuario, getHistorialCajasUsuario, getHistorialTransaccionesUsuario, cargando } = useAdminUsuarios();
	...
	const guardar = async () => {
		const respuesta = await actualizarUsuario(editando, formulario);
		if (respuesta !== null) {
			setEditando(null);
			cargar();
		}
	...
	const eliminar = async () => {
		const respuesta = await eliminarUsuario(confirmarEliminarId);
	...
	const verHistorial = async (id, tipo = 'cajas') => {
		...
		const datos =
			tipo === 'cajas' ? await getHistorialCajasUsuario(id) : await getHistorialTransaccionesUsuario(id);
	...
	const cambiarHistorialTab = async (tipo) => {
		...
		const datos =
			tipo === 'cajas'
				? await getHistorialCajasUsuario(historialId)
				: await getHistorialTransaccionesUsuario(historialId);
		setHistorialDatos(datos ?? []);
	};

	if (cargando && usuarios.length === 0) return <Cargando />;
			...
			<div className="admin-seccion">
					...
					<div className="admin-modal-overlay" onClick={() => setHistorialId(null)}>
						<div className="admin-modal" onClick={(e) => e.stopPropagation()}>
							<div className="admin-modal-header">
								...
								<button className="admin-modal-cerrar" onClick={() => setHistorialId(null)}>
									✕
								</button>
							</div>
							<div className="admin-modal-tabs">
								<button
									className={historialTab === 'cajas' ? 'activo' : ''}
									onClick={() => cambiarHistorialTab('cajas')}
								>
									Cajas abiertas
								</button>
								<button
									className={historialTab === 'transacciones' ? 'activo' : ''}
									onClick={() => cambiarHistorialTab('transacciones')}
								>
									Transacciones
								</button>
							</div>
					...
					<div className="admin-form-inline">
						...
						<div className="admin-form-grid">
							...
							<CampoForm
								...
								onChange={(v) => setFormulario({ ...formulario, email: v })}
								type="email"
							/>
							<CampoForm
								label="Saldo (KC)"
								value={formulario.saldo}
								onChange={(v) => setFormulario({ ...formulario, saldo: Number(v) })}
								type="number"
							/>
							<div className="campo-form">
								<label>VIP</label>
								<select
									value={formulario.suscripcion ? 'si' : 'no'}
									onChange={(e) => setFormulario({ ...formulario, suscripcion: e.target.value === 'si' })}
								>
						...
						<div className="admin-form-acciones">
							<button className="btn-admin-cancelar" onClick={() => setEditando(null)}>
								Cancelar
							</button>
							...
							{usuarios.map((u) => (
								<tr key={u.id}>
									...
									<td className="td-acciones">
										<button className="btn-accion btn-editar" onClick={() => abrirEdicion(u)}>
											Editar
										</button>
										<button className="btn-accion btn-historial" onClick={() => verHistorial(u.id)}>
											Historial
										</button>
										<button className="btn-accion btn-eliminar" onClick={() => setConfirmarEliminarId(u.id)}>
											Eliminar
										</button>
									</td>
								</tr>
				...
				{paginacion && paginacion.last_page > 1 && (
					<div className="admin-paginacion">
						...
						<button disabled={pagina >= paginacion.last_page} onClick={() => setPagina(pagina + 1)}>
							Siguiente →
						</button>
					</div>
				)}
