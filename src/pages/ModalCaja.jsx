...
const ModalCaja = ({ caja, onCerrar }) => {
	...
	//paso hace: 'nada' → 'flash' (destello) → 'mostrar'
	...
	// Los objetos se muestran de mayor a menor probabilidad para que el usuario vea primero los más comunes. La probabilidad viene del pivot de la tabla caja_objeto.
	...
	return (
		<>
			...
			>
				<div className={`modal-caja${premio ? ' modal-caja-premio' : ''}`} onClick={(e) => e.stopPropagation()}>
					{premio ? (
						<>
							...
							<div className={`premio-contenedor${paso === 'mostrar' ? ' premio-visible' : ''}`}>
								...
								{/* Imagen con rayos y halo de luz */}
								...
								{premio.precio > 0 && <p className="premio-precio">{formatearKC(premio.precio)}</p>}
						...
						<>
							...
							<div className="modal-caja-der">
								<div className="modal-caja-boton">
									<button className="btn-abrir-modal" onClick={pedirConfirmacion} disabled={abriendo}>
								...
								<div className="modal-caja-lista-header">
									...
									{detalle && objetosOrdenados.length > 0 && (
										<span className="modal-caja-count">{objetosOrdenados.length} objetos</span>
									)}
								</div>
